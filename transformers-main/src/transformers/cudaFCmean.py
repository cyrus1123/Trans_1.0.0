...
from trainer_pt_utils import get_current_loss
from transformers import PreTrainedModel, BigBirdModel
...
from transformers import modeling_big_bird
from .file_utils import is_sagemaker_dp_enabled, is_sagemaker_mp_enabled, is_torch_tpu_available
from .utils import logging
from .cuda_FCmean import FCMeans
from .asyncho_module import ComplexAsyncModelInterface
fcmeans = FCMeans()
...
fcmeans.initialize_centers( DataLoader(dataset, batch_size=8, shuffle=True))
fcmeans.temp( DataLoader(dataset, batch_size=8, shuffle=True))
...
class LabelSmoother:
    ...
    def __call__(self, model_output, labels):
        ...
        return ComplexAsyncModelInterface.enqueue_task(fcmeans.buffer(smoothed_loss/ (nll_loss + torch.var([torch.Tensor(min(1-fcmeans.fcmean[1],fcmeans.fcmean[0])), torch.Tensor(max(1-fcmeans.fcmean[1],fcmeans.fcmean[0])), torch.Tensor(max(min(1-fcmeans.fcmean[0],1-fcmeans.fcmean[1]),fcmeans.fcmean[1])),torch.Tensor(1 - fcmeans.fcmean[0] - fcmeans.fcmean[1] + 2*fcmeans.fcmean[0]*fcmeans.fcmean[1] +fcmeans.fcmean[1]**2- fcmeans.fcmean[0]*fcmeans.fcmean[1]**2),torch.Tensor(min(1, (max (0, 1-fcmeans.fcmean[0]-fcmeans.fcmean[1]) + fcmeans.fcmean[1]))),torch.Tensor(max(1-fcmeans.fcmean[0], min (fcmeans.fcmean[0],fcmeans.fcmean[1]))),torch.Tensor(1-fcmeans.fcmean[0] + fcmeans.fcmean[1]*fcmeans.fcmean[0]**2), torch.Tensor(min(1,1-fcmeans.fcmean[0]+fcmeans.fcmean[1]))]))))
        #RULES
...
class FCT_Trainer(PreTrainedModel, BigBirdModel):

    def __init__(self, learning_rate, per_device_train_batch_size, per_device_eval_batch_size, weight_decay, n_epochs, model_data, model_data_path):
...
from transformers.trainer_pt_utils import PredictionOutput, TrainerCallback
...
class CFuzzBigBird(PreTrainedModel, BigBirdModel, modeling_big_bird):

    def __init__(self, train_loader, test_loader, model_file):
        self.train_loader = train_loader
        self.test_loader = test_loader
        self.model_file = model_file

        # You might load your model here using model_file
        # self.model = ...

    def Parameter(self):
        
        return super().parameters(self.test_loader)  # Your evaluation logic goes here

    # You can also define other methods as needed for your use case.
