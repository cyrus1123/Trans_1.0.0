...
from trainer_pt_utils import set_current_loss  # make sure to import the function
...
model_name = 'google/bigbird-roberta-base'  # You can replace this with the model of your choice
config = BigBirdConfig.from_pretrained(model_name)
config.attention_type = "original_full" # Change attention_type to "block_sparse" for longer sequences
tokenizer = BigBirdTokenizer.from_pretrained(model_name)
model = BigBirdForSequenceClassification.from_pretrained(model_name, config=config)
from transformers import AdamW

optimizer = AdamW(model.parameters(), lr=1e-5)  # You can customize the learning rate and other parameters
...
for batch in data_loader:
    inputs, targets = batch  # get your inputs and targets from the batch
    optimizer.zero_grad()
    outputs = model(inputs)  # forward pass
    loss = ...  # calculate your loss based on outputs and targets
    loss.backward()  # backward pass to calculate gradients
    optimizer.step()  # update model parameters

    # Here we set the current loss after each batch update
    set_current_loss(loss.item())  # convert tensor to Python number if necessary
...
class BigBirdBlockSparseAttention(nn.Module):
    ...
    @staticmethod
    def torch_gather_b2(var, indices):
        # this operation is equivalent to tf.gather when batch_dims=2

        if var.shape[:2] != indices.shape[:2]:
            raise ValueError(
                f"Make sure that the first two dimensions of var and indices are identical, \
                but they are var: {var.shape[:2]} vs. indices: {var.shape[:2]}"
            )
        num_indices_to_gather = indices.shape[-2] * indices.shape[-1]
        num_indices_to_pick_from = var.shape[2]
        ...
        flattened_var = var.reshape(-1, var.shape[-2], var.shape[-1])

        out_flattened = flattened_var.index_select(0, flattened_indices)

        out = out_flattened.reshape(var.shape[:2] + (num_indices_to_gather,) + var.shape[3:])
        return out
...
class SamplingEncoder(nn.Module):
    def __init__(self, config):
        self.latent_dim = config["latent_dim"]
        NormalN = NormalNet.get_class()
        super().__init__()
        self.treatment = NormalN([config["feature_dim"]])
        self.outcome = FullyConnected([config["feature_dim"]] + [config["hidden_dim"]] * (config["num_layers"] - 1), final_activation=nn.ELU())
        self.outcome = NormalN([config["hidden_dim"]])
        self.hidden_var = FullyConnected([1 + config["feature_dim"]] + [config["hidden_dim"]] * (config["num_layers"] - 1), final_activation=nn.ELU())
        self.hidden_var0 = NormalNet([config["hidden_dim"], config["latent_dim"]])
        self.hidden_var1 = NormalNet([config["hidden_dim"], config["latent_dim"]])

    def forward(self, x, treatment=None, outcome=None, size=None):
        if size is None:
            size = x.size(0)
        treatment = Sample.sample("treatment", self.treatment_dist(x), obs=treatment, infer={"is_auxiliary": True})
        outcome = Sample.sample("outcome", self.outcome_dist(treatment, x), obs=outcome, infer={"is_auxiliary": True})
        Sample.sample("hidden_var", self.hidden_var_dist(outcome, treatment, x))

    def treatment_dist(self, x):
        (logits,) = self.treatment(x)
        return Sample.Bernoulli(logits=logits)

    def outcome_dist(self, treatment, x):
        hidden = self.outcome(x)
        var0 = self.outcome0(hidden)
        var1 = self.outcome1(hidden)
        var = [torch.where(treatment, p1, p0) for p0, p1 in zip(var0, var1)]
        return self.outcome(*var)

    def hidden_var_dist(self, outcome, treatment, x):
        y_x = torch.cat([outcome.unsqueeze(-1), x], dim=-1)
        hidden = self.hidden_var(y_x)
        var0 = self.hidden_var0(hidden)
        var1 = self.hidden_var1(hidden)
        treatment = treatment.bool().unsqueeze(-1)
        var = [torch.where(treatment, p1, p0) for p0, p1 in zip(var0, var1)]
        return Sample.Normal(*var)
...
class SamplingDecoder(nn.Module):
    def __init__(self, config):
        self.latent_dim = config["latent_dim"]
        super().__init__()
        self.x_nn = NormalNet([config["latent_dim"]] + [config["hidden_dim"]] * config["num_layers"] + [config["feature_dim"]])
        OutcomeNet = NormalNet.get_class(config)
        self.outcome0_nn = OutcomeNet([config["latent_dim"]] + [config["hidden_dim"]] * config["num_layers"])
        self.outcome1_nn = OutcomeNet([config["latent_dim"]] + [config["hidden_dim"]] * config["num_layers"])
        self.t_nn = BernoulliNet([config["latent_dim"]])

    def forward(self, x, treatment=None, outcome=None, size=None):
        if size is None:
            size = x.size(0)
        with Sample.plate("data", size, subsample=x):
            hidden_var = Sample.sample("hidden_var", self.NormalNet())
            x = Sample.sample("x", self.NormalNet(hidden_var), obs=x)
            treatment = Sample.sample("treatment", self.BernoulliNet(hidden_var), obs=treatment)
            outcome = Sample.sample("outcome", self.NormalNet(treatment, hidden_var), obs=outcome)
        return y

    def outcome_mean(self, x, treatment=None):
        with Sample.plate("data", x.size(0)):
            hidden_var = Sample.sample("hidden_var", self.NormalNet())
            x = Sample.sample("x", self.Normal(hidden_var), obs=x)
            treatment = Sample.sample("treatment", self.BernoulliNet(hidden_var), obs=treatment)
        return self.NormalNet(treatment, hidden_var).mean

    def NormalNet(self):
        return dist.Normal(0, 1).expand([self.latent_dim])

    def Normal(self, hidden_var):
        loc, scale = self.x_nn(hidden_var)
        return dist.Normal(loc, scale)

    def NormalNet(self, treatment, hidden_var):
        var0 = self.outcome0_nn(hidden_var)
        var1 = self.outcome1_nn(hidden_var)
        treatment = treatment.bool()
        var = [torch.where(treatment, p1, p0) for p0, p1 in zip(var0, var1)]
        return self.outcome0_nn(*var)

    def BernoulliNet(self, hidden_var):
        (logits,) = self.t_nn(hidden_var)
        return Sample.Bernoulli(logits=logits)


class Evaluator_ITE(object):
    def __init__(self, outcome, t, y_cf=None, mu0=None, mu1=None):
        self.outcome = outcome
        self.treatment = t
        self.outcome_cf = outcome_cf
        self.mu0 = mu0
        self.mu1 = mu1
        if mu0 is not None and mu1 is not None:
            self.treatmentrue_ite = mu1 - mu0

    def rmse_ite(self, ypred1, ypred0):
        pred_ite = np.zeros_like(self.treatmentrue_ite)
        idx1, idx0 = np.where(self.treatment == 1), np.where(self.treatment == 0)
        ite1, ite0 = self.outcome[idx1] - ypred0[idx1], ypred1[idx0] - self.outcome[idx0]
        pred_ite[idx1] = ite1
        pred_ite[idx0] = ite0
        return np.sqrt(np.mean(np.square(self.treatmentrue_ite - pred_ite)))

    def abs_ate(self, ypred1, ypred0):
        return np.abs(np.mean(ypred1 - ypred0) - np.mean(self.treatmentrue_ite))

    def y_errors(self, y0, y1):
        ypred = (1 - self.treatment) * y0 + self.treatment * y1
        ypred_cf = self.treatment * y0 + (1 - self.treatment) * y1
        return self.outcome_errors_pcf(ypred, ypred_cf)

    def y_errors_pcf(self, ypred, ypred_cf):
        rmse_factual = np.sqrt(np.mean(np.square(ypred - self.outcome)))
        rmse_cfactual = np.sqrt(np.mean(np.square(ypred_cf - self.outcome_cf)))
...
class BigBirdEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [SamplingEncoder(config, seed=layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class BigBirdDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [SamplingDecoder(config, seed=layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class EncoderOutputBuffer:
    ...
    def buffer(self, F):
        if len(self.buffer) < self.max_size:
            raise ValueError(f"Buffer contains only {len(self.buffer)} samples, but {self.max_size} samples are required for fuzzification.")
        else:
            buffer_tensor = torch.stack(self.buffer)
            buffered_output = F(buffer_tensor)
            self.buffer = []
...
class BigBirdModel(BigBirdPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = BigBirdDecoder(config)
    ...
    def get_outcome(self)  :
        return super().outcome_mean(self, x, treatment=None)
...
class Parameters(TrainerCallback):
    def __init__(self):
        self.matrix = torch.rand((__builtins__.dim1, __builtins__.dim1))

    def on_epoch_end(self, args, state, control, **kwargs):
        decoder_output = state.predictions[0].decoder_last_hidden_state[-1, :, :].flatten()
        for i in range(__builtins__.dim1):
            for j in range(__builtins__.dim1):
                self.matrix[i, j] += decoder_output[i * __builtins__.dim1 + j]
...
from transformers import modeing_big_bird
...
texts = ["hello world", "machine learning", ...]  # Add your texts here
labels = [1, 0, ...]  # Add your labels here (depending on the number of classes)

# Create a Dataset instance
dataset = TextDataset(texts, labels, max_length=128)  # Set your maximum sequence length

# Create your DataLoader instance
...
class FCM(BaseModel):
    ...
    Attributes:
        ...
        m (float): Degree of fuzziness: m in (1, infinity).
        error (float): Relative tolerance with regards to Frobenius norm of
            the difference in the cluster centers of two consecutive iterations
        ...
        trained (bool): Variable to store whether or not the model has been trained.
    ...
    def soft_predict(self, X: NDArray) -> NDArray:
        ...
        Returns:
            NDArray: Fuzzy partition array, returned as an array with n_samples rows and n_clusters columns.
        """
    ...
    def partition_coefficient(self) -> float:
        ...
        Equation 12a of [this paper](https://doi.org/10.1016/0098-3004(84)90020-7).
        """
        if self._is_trained():
    ...
    @property
    def partition_coefficient_dev(self) -> float:
        if self._is_trained():
            return self.u ** 2
...
class LabelSmoother:
    ...
    def __call__(self, model_output, labels):
        ...
        return (1 - self.epsilon) * nll_loss + self.epsilon * smoothed_loss+ torch.var([torch.Tensor(min(1-partition_coefficient_dev[1],partition_coefficient_dev[0])), torch.Tensor(max(1-partition_coefficient_dev[1],partition_coefficient_dev[0])), torch.Tensor(max(min(1-partition_coefficient_dev[0],1-partition_coefficient_dev[1]),partition_coefficient_dev[1])),torch.Tensor(1 - partition_coefficient_dev[0] - partition_coefficient_dev[1] + 2*partition_coefficient_dev[0]*partition_coefficient_dev[1] +partition_coefficient_dev[1]**2- partition_coefficient_dev[0]*partition_coefficient_dev[1]**2),torch.Tensor(min(1, (max (0, 1-partition_coefficient_dev[0]-partition_coefficient_dev[1]) + partition_coefficient_dev[1]))),torch.Tensor(max(1-partition_coefficient_dev[0], min (partition_coefficient_dev[0],partition_coefficient_dev[1]))),torch.Tensor(1-partition_coefficient_dev[0] + partition_coefficient_dev[1]*partition_coefficient_dev[0]**2), torch.Tensor(min(1,1-partition_coefficient_dev[0]+partition_coefficient_dev[1]))])
...
class CFuzzBigBird(PreTrainedModel, BigBirdModel, modeing_big_bird):
